...
export const useLeadIn = (): [0 | 3 | 5 | 10, (value: 0 | 3 | 5 | 10) => Promise<void>] => {
  ...
  const setLeadIn = async (value: 0 | 3 | 5 | 10) => {
    if (settings?.id) {
      ...
      await db.settings.add({ id: 1, leadIn: value, unit: 'kg', sound: true, haptics: true, restDefaults: { straight: 90, superset: 60, circuit: 45 }, ownedKettlebells: [16, 20, 24, 32], sideSwitchSec: 5 })
    }
  }
